...
func NewEnsurer(logger logr.Logger) genericmutator.Ensurer {
	return &ensurer{
		logger: logger.WithName("ionos-controlplane-ensurer"),
	}
}
...
func (e *ensurer) ensureChecksumAnnotations(ctx context.Context, template *corev1.PodTemplateSpec, namespace string) error {
	return controlplane.EnsureConfigMapChecksumAnnotation(ctx, template, e.client, namespace, ionos.CloudProviderConfig)
}
...
func (e *ensurer) EnsureKubeletCloudProviderConfig(ctx context.Context, gctx gcontext.GardenContext, kubeletVersion *semver.Version, data *string, namespace string) error {
	...
	err := e.client.Get(ctx, kutil.Key(namespace, ionos.CloudProviderConfig), &cm)
	if err != nil {
		if errors.IsNotFound(err) {
			e.logger.Info("configmap not found", "name", ionos.CloudProviderConfig, "namespace", namespace)
			return nil
		}
		return fmt.Errorf("could not get configmap '%s/%s': %w", namespace, ionos.CloudProviderConfig, err)
	}

	// Check if the data has "cloudprovider.conf" key
	if cm.Data == nil || cm.Data[ionos.CloudProviderConfigMapKey] == "" {
		return nil
	}

	// Overwrite data variable
	*data = cm.Data[ionos.CloudProviderConfigMapKey]
	return nil
}
